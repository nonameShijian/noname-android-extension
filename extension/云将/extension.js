...
return{
name:"云将",editable:false,content:function(config,pack){
/*主公AI*/
...
mode:["identity"],
filter:function(event,player){
return lib.config.extension_云将_shanheyonggu=='on'&&player.identity=='zhong'&&!player.isUnderControl(true);
},
content:function(){
if(get.mode()=='identity'){
var numf=game.countPlayer(function(current){
...
if(numz>=2){
  if(Math.random()>2/3){
    player.addSkill('shanheyonggu_zc');
  }
}
}
},
...
player.chooseTarget([1,player.hp],get.prompt2('wulie'),lib.filter.notMe).set("ai",function(target){
  if(get.mode()=='identity'&&player.hp>=4){
    if(player.identity=='nei'){
      if(Math.random()>1/5){
        return get.attitude(player,target)&&!target.hasSkillTag("maixie");
      }
    }else{
...
/*AI禁将*/
if(lib.config.fengyunzaiqi2&&lib.config.baihualiaoluan2){
var savedFilter=lib.filter.characterDisabled;
lib.filter.characterDisabled=function(i,libCharacter){
if(lib.character[i]&&lib.character[i][0]!='female'){
return true;
}
if(i&&i.indexOf('yun')!=0){
if(i&&i.indexOf('db_yun')!=0){
return true;
}
}
...
return lib.config.baihualiaoluan2&&player.isUnderControl(true)&&player==game.me;
},
content:function(){
'step 0'
...
player.reinit(name,'yunyouzi',false);
player.changeGroup('shen');
if(_status.characterlist){
_status.characterlist.add(name);
_status.characterlist.remove('yunyouzi');
}
}
},
}
...
superCharlotte:true,
filter:function(event,player){
return lib.config.extension_云将_yishifenghua=='on'&&lib.config['extension_十周年UI_borderLevel']=='kill';
},
content:function(){
...
filter:function(event,player){
return lib.config.extension_云将_yishifenghua=='on'&&lib.config['extension_十周年UI_borderLevel']=='kill'&&player.storage.biankuang<5;
},
forced:true,
unique:true,
popup:false,
...
fengyunzaiqi:{
  "name":"风云再起",
  "intro":"保存当前AI禁选配置并强制将所有非本拓展或非本人制作的武将配置为AI禁选，关闭后恢复保存的配置，重启生效。（注意：若与其他扩展的禁用武将功能同时打开会导致无法选将）",
...
baihualiaoluan:{
  "name":"百花缭乱",
  "intro":"保存当前AI禁选配置并强制将所有男性武将配置为AI禁选，身份模式下可召唤一只帅气的狗子，关闭后恢复保存的配置，重启生效。（注意：若与其他扩展的禁用武将功能同时打开会导致无法选将）",
  "init":lib.config.baihualiaoluan2===undefined?false:lib.config.baihualiaoluan2,
  onclick:function(item){
    game.saveConfig('baihualiaoluan2',item);
    game.saveConfig('extension_云将_baihualiaoluan',item);
  }
},
yishifenghua:{
  name:'一世风华',
  intro:'安装并打开千幻聆音，部分武将触发对应技能后会切换隐藏皮肤。<br><br>安装并打开十周年UI，武将将边框设置为随武将评级时，武将边框会随击杀数升级。',
  init:'on',
  item:{
    'on':'开',
    'off':'关',
  ...
  onclick:function(item){
    game.saveConfig('extension_云将_yishifenghua',item);
  }
},
shanheyonggu:{
  name:'山河永固',
  intro:'全扩AI增强。（注意：若与其他拓展AI增强功能同时打开可能会导致AI错乱）',
  init:'on',
  item:{
    'on':'开',
    'off':'关',
...
character:{
  character:{
    ...
    yunguojia:["male","wei",3,["yunguimou","yunyice"],["des:“如若奉孝在，何使孤至此……痛哉！奉孝……惜哉！奉孝……”———宇宙战神的粉丝"]],
    yuncaocao:["male","wei",3,["yunxionglve","yunxieling","yunzongyu"],["zhu","des:今起义兵，以除暴乱；九合诸侯，一扫群桀！"]],
    yunlvbu:["male","qun",5,["yuntanlang","yunwushuang"],["zhu","des:“那是……是刘虎哒！！！刘虎出阵了！！” “快逃！！快！这种怪物我们怎么可能……” “鬼神！鬼神降临在虎牢关！！！神鬼惊怖！！！”"]],
    yunwangji:["male","wei",3,["yunqizhi"],["des:别特么再问吴懿跟我有什么关系了，我们不熟，也没有任何绯闻！"]],
    ...
    yunhuangyueying:["female","shu",3,["yunlinglong"],["des:孔明大人，待扶汉功成，我们便回隆中……可好……"]],
    ...
    yuncaoying:["female","wei",3,["yunlingren","yunfujian"],["des:老将军虎威犹在，可惜命不久矣！<br>将军一副好骨，不如埋于此山？<br>将军这些把戏，可难不倒我哦。<br>我已在此，等候将军多时了！<br>你这点小心思，我还会猜不透？<br>你输了，这些就要归我。<br><br>———曹婴的世界只有将军(๑>؂<๑）！！！"]],
    yunzuoci:["male","qun",3,["yunyugui"],["des:谁说化身不算女装？"]],
    ...
    yuntaishici:["male","wu",4,["yuntianyi","yunaozhan"],["des:周瑜：“为什么会变成这样呢……第一次，称霸了江东，第一次交到了一生的挚友。这两件愉快的事情交织在了一起。而这两份喜悦，又会给我带来许许多多的喜悦。我本应该获得了这种如梦一般的幸福时光才对。可是，为什么，会变成现在这样呢……明明是我先来的，认识伯符也好，一起战斗也好……”<br><br>太史慈：“伯符，速与我一战！”"]],
},
  characterSort:{
    "mode_extension_云将":{
...
var list=[];
  ...
  var target=list.randomGet();
    player.storage.shanheyonggu=target;
    ...
    effect:{
      player:function(card,player,target){
        if(get.tag(card,'damage')){
          ...
          if(target.getEquip('tengjia')){
            if(card.name=="sha"&&!player.getEquip('qinggang')){
            return "zeroplayertarget";
            }
            if(!card.nature){
            return "zeroplayertarget";
          ...
          if(target.getEquip('renwang')){
            if(get.color(card)=='black'&&card.name=="sha"&&!player.getEquip('qinggang')){
            return "zeroplayertarget";
            }
          }
        }
        ...
        if(card.name=="guohe"||card.name=="lebu"||card.name=="bingliang"||card.name=="caomu"||card.name=="zhujinqiyuan"||card.name=="caochuanjiejian"||card.name=="toulianghuanzhu"){
        if(target.identity=='fan') return [0,1];
        }
      }
    },
  },
  ...
  expose:1,
    effect:{
      player:function(card,player,target){
        ...
        if(card.name=="guohe"||card.name=="lebu"||card.name=="bingliang"||card.name=="caomu"||card.name=="zhujinqiyuan"||card.name=="caochuanjiejian"||card.name=="toulianghuanzhu"){
        if(target.identity=='zhong') return [0,1];
        if(game.zhu.hp>3&&target.identity=='zhu') return [0,1];
        }
      },
    },
  },
...
"shanheyonggu_sf":{
trigger:{
player:["useCardBefore","phaseUseBefore"],
},
silent:true,
forced:true,
popup:false,
content:function(){
game.countPlayer(function(current){
  if(current==player.storage.shanheyonggu){
    if(current.ai.shown<0.95) current.addExpose(1);
  }
});
},
},
...
if(player.identity=='fan'&&target.identity=='fan'&&target.countCards('h')>=3&&target.hp<=0&&get.attitude(player,target)>0){
  var numt=player.countCards('hs',{
    name:'tao'
  });
  var numx=target.hp;
  if((numx+numt)>0){
...
if(lib.config.extension_云将_shanheyonggu=='on'&&player.identity=='zhong'&&(target.identity=='zhong'||target.identity=='mingzhong'||target.identity=='zhu')&&target.hp<=0&&target!=player&&get.mode()=='identity'&&get.attitude(player,target)>0){
  var numt=player.countCards('hs',{
    name:'tao'
  });
  var numx=target.hp;
...
if(lib.config.extension_云将_shanheyonggu=='on'&&player.identity=='zhu'&&(target.identity=='zhong'||target.identity=='mingzhong')&&target.hp<=0&&player.hp>1&&get.mode()=='identity'&&get.attitude(player,target)>0){
  var numt=player.countCards('hs',{
    name:'tao'
  });
  var numx=target.hp;
...
if(lib.config.extension_云将_shanheyonggu=='on'&&player.identity=='zhu'&&target.identity=='nei'&&target.hp<=0&&player.hp>1&&numz==0&&numf>=2&&get.mode()=='identity'&&get.attitude(player,target)>0){
  var numt=player.countCards('hs',{
    name:'tao'
  });
  var numx=target.hp;
...
/*身份场反贼残局AI*/
if(lib.config.extension_云将_shanheyonggu=='on'&&get.mode()=='identity'){
if(get.tag(card,'damage')&&numz>=2&&numf==1){
  if(target.identity=='nei'){
    return "zeroplayertarget";
  }else{
  return [0,1];
...
if(player.identity=='zhu'&&player.hp>1){
  if((target.identity=='zhong'||target.identity=='mingzhong')&&target.hp<=0&&get.attitude(player,target)>0){
    var numt=player.countCards('hs',{
      name:'tao'
    })
    var numx=target.hp;
...
if(player.identity=='zhong'&&target!=player){
  if(target.identity=='zhu'){
    return [0,999]
  }
  if(target.identity=='zhong'||target.identity=='mingzhong'&&get.attitude(player,target)>0){
    var numt=player.countCards('hs',{
      name:'tao'
    })
    var numx=target.hp;
...
if(player.identity=='fan'){
  if(target.identity=='fan'&&target.hp<=0&&target.countCards('h')>=3&&get.attitude(player,target)>0){
    var numt=player.countCards('hs',{
      name:'tao'
    })
    var numx=target.hp;
...
target:function(card,player,target){
/*卖血AI*/
if(lib.config.extension_云将_shanheyonggu=='on'&&target.hasSkillTag("maixie")&&!target.hasSkillTag("maixie_defend")&&get.attitude(target,player)>0){
if(get.tag(card,'damage')){
  /*卖血VIP名单*/
  if(target.hasSkill('lingren_jianxiong')||target.hasSkill('new_rejianxiong')||target.hasSkill('jianxiong')||target.hasSkill('huituo')||target.hasSkill('hunzi')||target.hasSkill('olhunzi')||target.hasSkill('rehunzi')){
    return "zeroplayertarget";
  }
}
...
if(get.tag(card,'damage')&&(!player.hasSkillTag('jueqing')||!player.hasSkill('jiu'))){
  if(target.maxHp==target.hp||(target.countCards('hs',{
    name:'tao'
  })+target.hp)>=target.maxHp||(target.hp+target.hujia)>=target.maxHp){
...
/*闪相关AI*/
if(lib.config.extension_云将_shanheyonggu=='on'&&target.hasSkillTag("useShan")&&get.attitude(target,player)>=0){
if(card.name=="sha"&&player.hasSkill('jiu')){
  return "zeroplayertarget";
}
if(card.name=="sha"&&target.getEquip('bagua')&&player.getEquip('qinggang')){
  return "zeroplayertarget";
}
if(card.name=="sha"&&card.nature&&target.getEquip('tengjia')){
  return [0,-999];
...
/*藤甲仁王AI对策*/
if(lib.config.extension_云将_shanheyonggu=='on'&&(player.hasSkill('shanheyonggu_tz')||player.hasSkill('shanheyonggu_tf')||player.hasSkill('shanheyonggu_smz')||player.hasSkill('shanheyonggu_zc'))){
  if(card.name=="sha"&&!player.getEquip('qinggang')&&target.getEquip('tengjia')){
    return "zeroplayertarget";
  }
  if(card.name=="sha"&&!player.getEquip('qinggang')&&target.getEquip('renwang')){
    return "zeroplayertarget";
  }
  if(card.name=="sha"&&!card.nature&&target.getEquip('tengjia')){
    return "zeroplayertarget";
...
var cxdy=game.countPlayer(function(current){
  if(current!=player&&!current.getEquip('tengjia'))  return current.hp==1&&get.attitude(current,player)>0;
});
var cxdr=game.countPlayer(function(current){
  if(current!=player&&!current.getEquip('tengjia'))  return current.hp==1&&get.attitude(current,player)<=0;
});
/*南蛮万箭AI*/
if(lib.config.extension_云将_shanheyonggu=='on'&&(card.name=="nanman"||card.name=="wanjian")){
  if(cxdy>=cxdr){
  return "zeroplayertarget";
  }else{
  return [1,1];
...
if(lib.config.extension_云将_shanheyonggu=='on'&&(card.name=="nanman"||card.name=="wanjian")){
if(get.mode()=='identity'&&player.identity=='fan'&&game.zhu.hp==1){
  return [1,1];
}
}
if(lib.config.extension_云将_shanheyonggu=='on'&&(card.name=="nanman"||card.name=="wanjian")){
if(get.mode()=='identity'&&player.identity=='nei'&&game.zhu.hp>2){
  return [1,1];
}else{
  return "zeroplayertarget";
}
...
if(lib.config.extension_云将_shanheyonggu=='on'&&(card.name=="nanman"||card.name=="wanjian")){
if(Math.ceil(game.players.length)==2){
  return [1,1];
}
}
/*无懈AI*/
if(lib.config.extension_云将_shanheyonggu=='on'&&card.name=="wuxie"){
if(player.hasJudge('lebu')&&_status.currentPhase!=player){
  if(get.mode()=='identity'&&player.identity=='zhong'&&game.zhu.hp>2){
  return -1;
  }
...
if(player.hasJudge('bingliang')&&_status.currentPhase!=player){
  if(get.mode()=='identity'&&player.identity=='zhong'&&game.zhu.hp>2&&player.countCards('hs')<=2){
  return -1;
  }
  if(get.mode()=='identity'&&player.identity!='zhong'&&player.countCards('hs')<=2){
  return -1;
  }
  if(get.mode()!='identity'&&player.countCards('hs')<=2){
  return -1;
  }
}
}
...
if(lib.config.extension_云将_shanheyonggu=='on'&&player.hasSkill('jiu')&&card.name=="sha"&&(target.getEquip('baiyin')||target.getEquip('rewrite_baiyin'))&&get.attitude(target,player)<=0){
if(!player.getEquip('qinggang')){
  return "zeroplayertarget";
}
return [0,1];
}
if(lib.config.extension_云将_shanheyonggu=='on'&&player.hasSkill('jiu')&&card.name=="sha"){
...
if(lib.config.extension_云将_shanheyonggu=='on'&&card.name=="sha"&&card.nature&&target.isLinked()&&get.attitude(target,player)<=0){
  if(player.isLinked()&&(player.hp==1||(player.hp+player.hujia)<2)){
  ...
  if(player.isLinked()&&(target.hp>player.hp||target.hp>(player.hp+player.hujia))){
    if(card.nature=='fire'&&(!player.hasSkillTag('nofire')||!player.hasSkillTag('nodamage'))){
      return [0,-99];
    }
    if(card.nature=='thunder'&&(!player.hasSkillTag('nothunder')||!player.hasSkillTag('nodamage'))){
  ...
  if(player.isLinked()&&((player.hp+player.hujia)>3||player.hp>3)&&player.hasSkillTag('maixie')&&!player.hasSkillTag("maixie_defend")){
    return [0,1];
  }
  if(get.mode()=='identity'&&player.identity=='zhong'&&game.zhu.isLinked()){
    return [0,-99];
  }
  if(get.mode()=='identity'&&player.identity=='nei'&&numf>0&&(game.zhu.isLinked()||player.isLinked())){
    return [0,-99];
  }
  if(get.mode()=='identity'&&player.identity=='nei'&&numz>0&&game.zhu.isLinked()&&game.zhu.hp==1){
    return [0,-99];
  }
  if(hzdy>hzdr){
    return [0,-99];
...
if(lib.config.extension_云将_shanheyonggu=='on'&&card.name=="huogong"){
  if(target.isLinked()&&player.isLinked()&&(player.hp==1||(player.hp+player.hujia)<2)){
    if(!player.hasSkillTag('nofire')||!player.hasSkillTag('nodamage')){
      return "zeroplayertarget";
    }
    if(target.isLinked()&&player.isLinked()&&(target.hp>player.hp||target.hp>(player.hp+player.hujia))){
      if(!player.hasSkillTag('nofire')||!player.hasSkillTag('nodamage')){
        return "zeroplayertarget";
      }
      if(target.isLinked()&&player.isLinked()&&player.hp>=3&&player.hasSkillTag('maixie')&&!player.hasSkillTag("maixie_defend")){
        return [0,1];
      }
      if(target.isLinked()&&player.isLinked()&&player.hp>3&&hzdy<hzdr){
        return [0,1];
      }
      if(get.mode()=='identity'&&player.identity=='zhong'&&target.isLinked()&&game.zhu.isLinked()){
        return [0,-99]
      }
      if(get.mode()=='identity'&&player.identity=='nei'&&numf>0&&(game.zhu.isLinked()||player.isLinked())){
        return [0,-99];
...
/*拆顺AI*/
if(lib.config.extension_云将_shanheyonggu=='on'&&(card.name=="shunshou"||card.name=="guohe")&&get.attitude(player,target)<0&&target.maxHp>target.hp&&target.countCards('h')==0&&(target.getEquip('baiyin')||target.getEquip('rewrite_baiyin'))){
  return "zeroplayertarget";
}
if(lib.config.extension_云将_shanheyonggu=='on'&&(card.name=="shunshou"||card.name=="guohe")&&get.attitude(player,target)>0&&(target.hasJudge('bingliang')||target.hasJudge('lebu'))){
  return [1,1];
}
},
},
...
mode:["identity"],
filter:function(event,player){
return lib.config.extension_云将_tianxiasanfen=='on'&&player.group=='shu'&&get.itemtype(event.cards)=='cards'&&get.position(event.cards[0])=='d'&&!player.hasSkill('kongcheng');
},
content:function(){
...
if(result.control=='吴'){
  ...
  if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='db_yundaxiaoqiao'&&game.yjGetQhlySkin('db_yundaxiaoqiao')=='db_yundaxiaoqiao5.jpg'){
...
if(result.control){
  ...
  player.update();
  if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunhanyun'){
...
"step 4"
player.chooseToUse('使用一张牌或不使用令【神权】失效');
"step 5"
if(result.bool){
...
mp:{
  ...
  content:function(){
    ...
    if(player.hp<=num1){
      ...
      player.addTempSkill('yunchizhong',{
        player:'phaseZhunbeiBegin'
      }
      );
    }else{
...
content:function(){
'step 0'
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunlvmeng'){
...
filter:function(event,player){
if(event.player.hasSkill('yunfengyin')) return false;
...
trigger.player.addTempSkill('yunfengyin',{
player:'phaseAfter'
});
}
...
"yunsangdan":{
trigger:{
global:["damageEnd","loseHpEnd","gainMaxHpEnd"],
},
forced:true,
popup:false,
charlotte:true,
...
"yunsangdana":{
trigger:{
global:["dieAfter","loseMaxHpAfter","recoverAfter"],
},
forced:true,
popup:false,
charlotte:true,
...
content:function(){
player.draw(2);
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunsunshangxiang'){
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunsunshangxiang'&&game.yjGetQhlySkin('yunsunshangxiang')=='yunsunshangxiang9.jpg'){
...
player.addTempSkill('yunfengminga',{
player:'phaseZhunbeiEnd'
});
'step 1'
player.say(["落凤坡？！","大业未成，我岂能就此而亡!"].randomGet());
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunpangtong'&&!game.yjGetQhlySkin('yunpangtong')){
player.node.avatar.setBackgroundImage('extension/云将/yunpangtonga.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunpangtong'&&game.yjGetQhlySkin('yunpangtong')=='yunpangtong5.jpg'){
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaocao'&&!game.yjGetQhlySkin('yuncaocao')){
player.node.avatar.setBackgroundImage('extension/云将/yuncaocaoa.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaocao'&&game.yjGetQhlySkin('yuncaocao')=='yuncaocao4.jpg'){
player.node.avatar.setBackgroundImage('extension/云将/yuncaocao4a.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaocao'&&game.yjGetQhlySkin('yuncaocao')=='yuncaocao5.jpg'){
...
result:{
target:function(player,target){
if((target.identity=='zhong'||target.identity=='mingzhong')&&target.group!='wei'){
  return 10;
}
if(target.identity=='zhong'||target.identity=='mingzhong'){
return 5;
...
content:function(){
'step 0'
player.chooseTarget('是否发动【魏武】？',function(card,player,target){
...
if(event.targets.length){
  ...
  if(target.isAlive()){
    event.current.chooseToUse({
      name:'sha'
    },target,-1).set('prompt2','请选择：1. 对'+get.translation(target)+'使用一张杀；2.视为被 '+get.translation(player)+'使用一张杀');
  }
}else{
  event.finish();
}
...
if(player.storage.yunxionglve==true) return '出牌阶段，你可以弃置一张手牌获得一张锦囊牌。';
return '你使用【杀】指定目标／被【杀】指定为目标时，可弃置一张手牌获得目标／【杀】的使用者一张牌。';
},
},
group:['yunxionglve_xiongcai','yunxionglve_guixin_a','yunxionglve_guixin_b'],
subSkill:{
xiongcai:{
audio:"yunxionglve",
enable:"phaseUse",
...
prompt:"是否弃置一张牌发动【雄略】？",
filter:function(event,player,name){
return player.countCards('h')>0&&!player.storage.yunxionglve==true;;
},
content:function(){
'step 0'
if(player.countCards('h')<player.maxHp&&player.hasSkill('yunxionglve')){
  player.draw();
};
...
if(result.bool){
  player.gain(game.createCard(result.buttons[0].link[2]),'draw');
  player.changeZhuanhuanji('yunxionglve');
}
},
ai:{
order:10,
...
filter:function(event,player){
return event.target.countCards('he')>0&&event.target!=player&&player.countCards('h')>0&&event.card.name=='sha'&&player.storage.yunxionglve;
},
content:function(){
"step 0"
player.chooseToDiscard('h','弃置一张手牌',true).set('ai',function(card){
...
"step 1"
if(result.bool){
  player.logSkill('yunxionglve_guixin_a',trigger.target);
...
prompt:"是否弃置一张牌发动【雄略】？",
filter:function(event,player){
return event.player.countCards('he')>0&&player.countCards('h')>0&&event.card.name=='sha'&&player.storage.yunxionglve;
},
check:function(event,player){
return get.attitude(player,event.player)<=0;
},
...
"step 1"
if(result.bool){
  player.logSkill('yunxionglve_guixin_b',trigger.player);
  if(player.countCards('h')<player.maxHp){
    player.draw();
  }
  player.changeZhuanhuanji('yunxionglve');
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunguanyu'&&!game.yjGetQhlySkin('yunguanyu')){
  player.node.avatar.setBackgroundImage('extension/云将/yunguanyua.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunguanyu'&&game.yjGetQhlySkin('yunguanyu')=='yunguanyu4.jpg'){
...
if(get.attitude(player,event.target)>0&&event.target.hp==1&&(event.target.hujia>0||player.countCards('hs',{
  name:'tao'
})>=1)) return true;
return false;
},
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaishi'&&!game.yjGetQhlySkin('yuncaishi')){
  player.node.avatar.setBackgroundImage('extension/云将/yuncaishia.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaishi'&&game.yjGetQhlySkin('yuncaishi')=='yuncaishi4.jpg'){
  player.node.avatar.setBackgroundImage('extension/云将/yuncaishi4a.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaishi'&&game.yjGetQhlySkin('yuncaishi')=='yuncaishi8.jpg'){
...
hf:{
  ...
  popup:false,
  content:function(){
    if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaishi'){
...
if(player.storage.yunjueqing>0){
  ...
  if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhangchunhua'&&!game.yjGetQhlySkin('yunzhangchunhua')){
    player.node.avatar.setBackgroundImage('extension/云将/yunzhangchunhuaa.jpg');
  }
  if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhangchunhua'&&game.yjGetQhlySkin('yunzhangchunhua')=='yunzhangchunhua4.jpg'){
    player.node.avatar.setBackgroundImage('extension/云将/yunzhangchunhua4a.jpg');
  }
  if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhangchunhua'&&game.yjGetQhlySkin('yunzhangchunhua')=='yunzhangchunhua5.jpg'){
    player.node.avatar.setBackgroundImage('extension/云将/yunzhangchunhua5a.jpg');
  }
  if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhangchunhua'&&game.yjGetQhlySkin('yunzhangchunhua')=='yunzhangchunhua8.jpg'){
...
a:{
  ...
  content:function(){
    ...
    if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhangchunhua'){
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yundongbai'&&!game.yjGetQhlySkin('yundongbai')){
player.node.avatar.setBackgroundImage('extension/云将/yundongbaia.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yundongbai'&&game.yjGetQhlySkin('yundongbai')=='yundongbai2.jpg'){
player.node.avatar.setBackgroundImage('extension/云将/yundongbai2a.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yundongbai'&&game.yjGetQhlySkin('yundongbai')=='yundongbai5.jpg'){
player.node.avatar.setBackgroundImage('extension/云将/yundongbai5a.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yundongbai'&&game.yjGetQhlySkin('yundongbai')=='yundongbai7.jpg'){
player.node.avatar.setBackgroundImage('extension/云将/yundongbai7a.jpg');
}
'step 1'
player.awakenSkill('yunjiaoheng');
...
game.countPlayer(function(current){
  if(current.name=='yundongbai'&&current.countCards('h')<current.maxHp){
    player.line(current,'green');
    player.logSkill('yunchongni',event.targets);
    current.draw();
  }
});
},
...
prompt:function(event,player){
var str='';
str+='是否弃置装备区'+get.cnNumber(_status.event.player.countCards('e'))+'张牌对一名其他角色造成'+get.cnNumber(_status.event.player.countCards('e'))+'点火属性伤害？'
return str;
},
skillAnimation:true,
init:function(player){
...
content:function(){
"step 0"
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhouyu'&&!game.yjGetQhlySkin('yunzhouyu')){
player.node.avatar.setBackgroundImage('extension/云将/yunzhouyua.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhouyu'&&game.yjGetQhlySkin('yunzhouyu')=='yunzhouyu5.jpg'){
...
if(get.suit(event.card)&&player.countCards('he',{
...
target.addTempSkill('yunzhaoliea',{
player:'phaseAfter'
});
}
if(result.control=='咆哮'){
target.addTempSkill('yunzhaolieb',{
player:'phaseAfter'
});
}
if(result.control=='龙胆'){
target.addTempSkill('yunzhaoliec',{
player:'phaseAfter'
});
}
if(result.control=='铁骑'){
target.addTempSkill('yunzhaolied',{
player:'phaseAfter'
});
}
if(result.control=='烈弓'){
target.addTempSkill('yunzhaoliee',{
player:'phaseAfter'
});
}
},
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunliubei'&&!game.yjGetQhlySkin('yunliubei')){
...
result:{
target:function(player,target){
if((target.identity=='zhong'||target.identity=='mingzhong')&&target.group!='shu'){
  return 10;
}
if(target.identity=='zhong'||target.identity=='mingzhong'){
return 5;
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yundiaochan'&&!game.yjGetQhlySkin('yundiaochan')){
player.node.avatar.setBackgroundImage('extension/云将/yundiaochana.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yundiaochan'&&game.yjGetQhlySkin('yundiaochan')=='yundiaochan7.jpg'){
player.node.avatar.setBackgroundImage('extension/云将/yundiaochan7a.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yundiaochan'&&game.yjGetQhlySkin('yundiaochan')=='yundiaochan8.jpg'){
player.node.avatar.setBackgroundImage('extension/云将/yundiaochan8a.jpg');
}
},
mod:{
maxHandcard:function(player,num){
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yundiaochan'){
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunsunce'&&!game.yjGetQhlySkin('yunsunce')){
...
result:{
target:function(player,target){
if((target.identity=='zhong'||target.identity=='mingzhong')&&target.group!='wu'){
  return 10;
}
if(target.identity=='zhong'||target.identity=='mingzhong'){
return 5;
...
check:function(event,player){
var hs1=event.source.getCards('h','sha');
var hs2=player.getCards('h','sha');
if(get.attitude(player,event.source)<0&&(hs1.length<=hs2.length)){
  return 1.5;
}
...
game.countPlayer(function(current){
  if(current.name=='yunxushi'){
    ...
    if(lib.config.extension_云将_yishifenghua=='on'&&current.name=='yunxushi'){
...
content:function(){
'step 0'
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunxushi'&&!game.yjGetQhlySkin('yunxushi')){
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name=='db_yunjiaxu'){
...
a:{
  ...
  forced:true,
  content:function(){
    player.addTempSkill('yunweiwo',{
      player:'phaseZhunbeiBegin'
    });
    if(lib.config.extension_云将_yishifenghua=='on'&&player.name=='db_yunjiaxu'&&!game.yjGetQhlySkin('db_yunjiaxu')){
      player.node.avatar.setBackgroundImage('extension/云将/db_yunjiaxua.jpg');
    }
    if(lib.config.extension_云将_yishifenghua=='on'&&player.name=='db_yunjiaxu'&&game.yjGetQhlySkin('db_yunjiaxu')=='db_yunjiaxu5.jpg'){
...
forced:true,
filter:function(event,player){
return event.getParent(2).name!='yunlinglong';
},
content:function(){
'step 0'
player.chooseTarget('请选择【玲珑】摸牌的目标','令一名角色摸一张牌').set('ai',function(target){
return get.attitude(_status.event.player,target)*Math.sqrt(Math.max(1,4-target.countCards('h')));
});
'step 1'
if(result.bool){
...
player.line(target,'green');
target.draw();
if(player.storage.qiqiao_a!=1){
player.storage.qiqiao++;
player.storage.qiqiao_a=1;
}
}
},
mod:{
maxHandcard:function(player,num){
return num+player.storage.qiqiao;
},
},
group:["yunlinglong_a","yunlinglong_b","yunlinglong_discard","yunlinglong_equip"],
subSkill:{
discard:{
audio:"ext:云将/audio:2",
trigger:{
...
forced:true,
filter:function(event,player){
return event.getParent(3).name!='yunlinglong_discard'&&event.cards2.length>0&&_status.currentPhase!=player;
},
content:function(){
'step 0'
player.chooseTarget('请选择【玲珑】弃置牌的目标','弃置一名角色的一张牌',function(card,player,target){
...
player.say(["七巧玲珑心！","夫君，切勿操劳。"].randomGet());
player.discardPlayerCard(1,result.targets[0],'hej');
if(player.storage.qiqiao_b!=1){
player.storage.qiqiao++;
player.storage.qiqiao_b=1;
}
}
},
ai:{
...
forced:true,
filter:function(event,player){
return event.getParent(1).name!='yunlinglong_equip'&&_status.currentPhase==player;
},
content:function(){
'step 0'
player.chooseTarget('请选择【玲珑】恢复体力的目标','令一名角色恢复一点体力',function(card,player,target){
...
player.line(target,'green');
target.recover();
if(player.storage.qiqiao_c!=1){
player.storage.qiqiao++;
player.storage.qiqiao_c=1;
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunhuangyueying'){
...
content:function(){
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunhuangyueying'&&!game.yjGetQhlySkin('yunhuangyueying')){
  player.node.avatar.setBackgroundImage('extension/云将/yunhuangyueyinga.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunhuangyueying'&&game.yjGetQhlySkin('yunhuangyueying')=='yunhuangyueying5.jpg'){
  player.node.avatar.setBackgroundImage('extension/云将/yunhuangyueying5a.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunhuangyueying'&&game.yjGetQhlySkin('yunhuangyueying')=='yunhuangyueying6.jpg'){
  player.node.avatar.setBackgroundImage('extension/云将/yunhuangyueying6a.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunhuangyueying'&&game.yjGetQhlySkin('yunhuangyueying')=='yunhuangyueying9.jpg'){
...
trigger:{
  ...
  superCharlotte:true,
  init:function(player){
  player.storage.qiqiao=0;
  },
  content:function(){
  player.removeMark('qiqiao',Infinity);
  player.storage.qiqiao_a=0;
  player.storage.qiqiao_b=0;
  player.storage.qiqiao_c=0;
  },
  sub:true,
},
},
...
return !target.hasSkill('yunwangzuo_off');
}).set('ai',function(target){
return get.attitude(player,target);
var att=get.attitude(player,target);
...
target.addTempSkill('yunjinnang',{
player:'shouji'
});
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunsimayi'&&game.yjGetQhlySkin('yunsimayi')=='yunsimayi4.jpg'){
player.node.avatar.setBackgroundImage('extension/云将/yunsimayi4a.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunsimayi'&&game.yjGetQhlySkin('yunsimayi')=='yunsimayi5.jpg'){
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunsunquan'&&!game.yjGetQhlySkin('yunsunquan')){
  player.node.avatar.setBackgroundImage('extension/云将/yunsunquana.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunsunquan'&&game.yjGetQhlySkin('yunsunquan')=='yunsunquan5.jpg'){
player.node.avatar.setBackgroundImage('extension/云将/yunsunquan5a.jpg');
}
},
ai:{
order:7,
...
if(result.bool){
  ...
  event.target=result.targets[0];
player.chooseButton(['选择'+get.translation(event.target)+'的一张牌标记为〖间〗', event.target.getCards('h')]).set('ai', function(button){
  return get.attitude(player,event.target)<=0;
  var target=event.target;
...
mod:{
maxHandcard:function(player,num){
if(num<player.maxHp) return num=player.maxHp;
},
},
},
"yunlingren":{
...
'step 0'
event.count = 0;
player.chooseTarget(get.prompt('yunlingren'),'是否选择一名目标角色猜测其手牌构成？',function(card,player,target){
return _status.event.targets.contains(target);
}).set('ai',function(target){
...
var dialog=ui.create.dialog('〖凌人〗','猜测'+get.translation(target)+'的手牌构成',[list,'vcard']);
for(var i=0;i<3;i++){
var dd=document.querySelector('[data-card-name="'+list[i][2]+'"]');
dd.style['background-size']="100% 100%";
/* dd.setBackgroundImage('extension/十周年/image/card/'+list[i][2]+'.webp');*/ 

var imgObj=new Image(105,150);
imgObj.src=lib.assetURL+"extension/云将/biaoqian/"+list[i][2]+".png";
dd.appendChild(imgObj);
};
...
if(event.num==3&&lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaoying'&&!game.yjGetQhlySkin('yuncaoying')){
  if(Math.random()>1/2){
...
if(event.num==3&&lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaoying'&&game.yjGetQhlySkin('yuncaoying')=='yuncaoying1.jpg'){
  if(Math.random()>1/2){
...
if(event.num==3&&lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaoying'&&game.yjGetQhlySkin('yuncaoying')=='yuncaoying2.jpg'){
  if(Math.random()>1/2){
...
if(event.num==3&&lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaoying'&&game.yjGetQhlySkin('yuncaoying')=='yuncaoying3.jpg'){
  if(Math.random()>1/2){
...
if(event.num==3&&lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaoying'&&game.yjGetQhlySkin('yuncaoying')=='yuncaoying4.jpg'){
  if(Math.random()>1/2){
...
if(event.num==3&&lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaoying'&&game.yjGetQhlySkin('yuncaoying')=='yuncaoying5.jpg'){
  if(Math.random()>1/2){
...
if(event.num==3&&lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaoying'&&game.yjGetQhlySkin('yuncaoying')=='yuncaoying7.jpg'){
  if(Math.random()>1/2){
...
if(event.num==3&&lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaoying'&&game.yjGetQhlySkin('yuncaoying')=='yuncaoying8.jpg'){
  if(Math.random()>1/2){
...
if(event.num==3&&lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaoying'&&game.yjGetQhlySkin('yuncaoying')=='yuncaoying9.jpg'){
  if(Math.random()>1/2){
...
var banlist=['zhaoxiang','tw_zhaoxiang','shen_luxun','zhangji','re_sunyi','yanghuiyu','re_sunxiu','yunsunshangxiang','old_lingju','yundongbai','re_baosanniang','sp_pangtong','shen_jiangwei','yanpu','yunjiangwei','yunzhaoxiang','yuncaoying','yunzuoci','yunlvmeng','yunxushi','yunsimayi'];
if(banlist.contains(i)) continue;
list.push(i);
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunjiangwei'&&!game.yjGetQhlySkin('yunjiangwei')){
  player.node.avatar.setBackgroundImage('extension/云将/yunjiangweia.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunjiangwei'&&game.yjGetQhlySkin('yunjiangwei')=='yunjiangwei5.jpg'){
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhangjiao'&&!game.yjGetQhlySkin('yunzhangjiao')){
  player.node.avatar.setBackgroundImage('extension/云将/yunzhangjiaoa.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhangjiao'&&game.yjGetQhlySkin('yunzhangjiao')=='yunzhangjiao7.jpg'){
  if(Math.random()>1/2){
...
if(result.suit=='spade'){
  ...
  if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaiwenji'){
...
if(result.suit=='heart'){
  ...
  if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaiwenji'&&!game.yjGetQhlySkin('yuncaiwenji')){
    player.node.avatar.setBackgroundImage('extension/云将/yuncaiwenjia.jpg');
  }
  if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaiwenji'&&game.yjGetQhlySkin('yuncaiwenji')=='yuncaiwenji5.jpg'){
...
effect:{
player:function(card,player,target){
if(card.name=='huogong'||card.name=='tao'||card.name=='jiu'){
  return [1,1];
}
},
},
...
popup:false,
prompt:"是否发动【谦雅】弃置一张牌令一名其他角色立即执行一次额外的弃牌阶段？",
content:function(){
'step 0'
player.logSkill('yunqianya');
player.chooseToDiscard('h',true).set('ai',function(card){
...
if(result.bool){
  var num=player.maxHp-player.hp;
  if(player.isDamaged()&&player.hujia<num){
    player.changeHujia();
  }
...
if(result.bool){
  ...
  var num=Math.min(player.hp,trigger.cards.length);
  player.discardPlayerCard(target,'he',[1,num],true);
  target.addTempSkill('yunguose_xz',{
    player:'phaseAfter'
  });
}
},
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaopi'&&!game.yjGetQhlySkin('yuncaopi')){
  player.node.avatar.setBackgroundImage('extension/云将/yuncaopia.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yuncaopi'&&game.yjGetQhlySkin('yuncaopi')=='yuncaopi4.jpg'){
...
if(player.hp>3){
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhaoxiang'&&game.yjGetQhlySkin('yunzhaoxiang')=='yunzhaoxiang4.jpg'){
  player.node.avatar.setBackgroundImage('extension/云将/yunzhaoxiang4a.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunzhaoxiang'&&game.yjGetQhlySkin('yunzhaoxiang')=='yunzhaoxiang5.jpg'){
...
player.chooseBool('是否摸'+num+'张牌并令'+get.translation(trigger.player)+'受到的此次伤害加'+num+'？').set('ai',function(event,player){
...
if(event.card&&event.card.name=="chenhuodajie") return false;
...
if(event.targets.length){
  event.current=event.targets.shift();
  var card=game.createCard(trigger.card.name,trigger.card.suit,trigger.card.number,trigger.card.nature);
  if(trigger.target.isAlive()&&(event.current.canUse({
    name:trigger.card.name
},trigger.target,false,false)||lib.filter.targetEnabled2(trigger.card,event.current,trigger.target,false,false))){
...
intro:{
name:"定策",
content:"你回合内使用非转化牌时随机执行一项<br>①摸一张牌<br>②恢复一点体力<br>③此牌不可响应<br>④此回合使用【杀】次数加一",
},
trigger:{
player:"useCard",
},
...
content:function(){
'step 0'
if(player.countMark('yunxiaolie')<3){
...
effect:{
player:function(card,player,target){
if(get.color(card)=='black'&&player!=target&&target.hp>player.hp&&get.attitude(player,target)<0) return [1,1];
},
},
},
},
...
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunhuaman'&&!game.yjGetQhlySkin('yunhuaman')){
player.node.avatar.setBackgroundImage('extension/云将/yunhuamana.jpg');
}
if(lib.config.extension_云将_yishifenghua=='on'&&player.name1=='yunhuaman'&&game.yjGetQhlySkin('yunhuaman')=='yunhuaman4.jpg'){
player.node.avatar.setBackgroundImage('extension/云将/yunhuamana.jpg');
}
'step 1'
player.draw(5);
...
if(target.sex=='male'){
  ...
  player.addTempSkill('yunzhanyuan_bh',{
    player:'zhanyuan'
  });
  target.changeGroup('shu');
  if(target.identity!=='zhu'){
  ...
  target.addTempSkill('yunzhanyuan_bh',{
    player:'zhanyuan'
  });
}
...
target.addTempSkill('yunzhanyuan_xc',{
  player:'zhanyuan'
});
}
...
superCharlotte:true,
filter:function(event,player){
return !player.hasSkill('yunshiguan');
},
content:function(){
var num=Math.floor(player.countCards('h')/2);
if(player.isMaxHandcard(true)){
...
if(player.group=='qun'&&!player.hasSkill('yunshiguana_ls')){
  event.trigger('lisi');
  player.addTempSkill('yunshiguana_ls');
}
},
sub:true,
},
...
player.chooseTarget([1,num],'是否发动【雄桀】选择至多'+num+'名角色？<br>视为对这些角色使用一张【万箭齐发】',function(card,player,target){
return player!=target;
}).set("ai",function(target){
return -get.attitude(player,target);
});
...
delete player.getStat('skill').yunxiongjie;
}
},
},
"yunlisi":{
...
if(target.hp>=2) return target.hp*att;
if(target.group=='qun'&&target.hp>=2) return target.hp*att+1;
});
"step 1"
if(result.bool){
player.logSkill('yunlisi');
var target=result.targets[0];
game.log(target,'成为了','【立嗣】','的目标');
target.addTempSkill('yunshiguana',{
player:'phaseAfter'
});
}
},
...
effect:{
  player:function(card,player,target){
    if(get.attitude(target,player)<0&&card.name=="sha"&&!target.hasSkill('yuntianyi_b')){
      return [1,1]
      if(!card.nature&&target.getEquip('tengjia')){
        return "zeroplayertarget";
      }
      if(get.color(card)=='black'&&target.getEquip('renwang')){
        return "zeroplayertarget";
      }
    }
  },
...
filter:function(event,player){
var num=game.countPlayer(function(current){
return current.hasSkill('yuntianyi_b');
});
return event.card&&event.card.name=='sha'&&num>=1;
},
content:function(){
'step 0'
var num=game.countPlayer(function(current){
...
player.chooseTarget('是否发动【天义】指定额外角色成为此【杀】的目标？',[1,num],function(card,player,target){
...
playerEnabled:function(card,player,target){
  if(card.name=='sha'&&target.hasSkill('yuntianyi_d')) return false;
},
},
},
"yuntianyi_d":{
...
mod:{
cardUsable:function(card,player,num){
var nums=game.countPlayer(function(current){
return current.hasSkill('yuntianyi_b');
});
if(card.name=='sha'){
return num+=Math.min(2,nums);
}
},
},
group:['yunaozhan_pd'],
...
yunshenzi:"神姿",
"yunshenzi_info":"一回合限一次：场上有角色受到伤害后，你选择一项：一、摸一张牌；二、获得其一个技能。",
yunshenquan:"神权",
"yunshenquan_info":"出牌阶段，你可观看一名其他角色的手牌并获得其一张牌，然后你须立即使用一张牌，否则【神权】此回合失效。",
yunjilve:"极略",
"yunjilve_info":"每回合一种牌名限一次：你可以将一张牌当做任意基本牌或锦囊牌使用或打出。",
yunyice:"遗策",
...
yunyongzhi:"勇智",
"yunyongzhi_info":"结束阶段：你摸X张牌，若X不小于三，则额外获得一张【酒】；你使用【酒】无次数限制。(X为你此回合造成的伤害值，最多为三)",
yunbiyue:"闭月",
...
yunjimou:"极谋",
"yunjimou_info":"锁定技：准备阶段／结束阶段，你获得【控局】直到回合结束／你获得【帷幄】直到回合开始。<br><br>【控局】：你使用红色的基本牌或普通锦囊牌可额外结算一次。<br><br>【帷幄】：锁定技：其他角色使用的黑色牌不能指定你为目标。",
yunjiance:"间策",
...
yunxionglve:"雄略",
"yunxionglve_info":"转换技：发动【雄略】时若手牌数小于体力上限，你摸一张牌。阴、你使用【杀】指定目标／被【杀】指定为目标时，可弃置一张手牌获得目标／【杀】的使用者一张牌；阳、出牌阶段，你可以弃置一张手牌获得一张锦囊牌。",
yunlongdan:"龙胆",
"yunlongdan_info":"你可将任意一张手牌当做【杀】／【闪】使用或打出；出牌阶段你使用【杀】的次数额外加X。（X为你已损失的体力值，最多增加至三次）",
yunlonglie:"龙烈",
"yunlonglie_info":"锁定技：你每回合发动【龙胆】首次使用／打出一种花色的牌后，若为：♠️你可令一名其他角色受到一点伤害；♥️你恢复一点体力；♣️你可弃置一名其他角色一张牌；♦️你摸两张牌。",
yunlinglong:"玲珑",
"yunlinglong_info":"锁定技；①你回合内不因【玲珑】恢复体力后，可令一名角色恢复一点体力；②你不因【玲珑】获得牌后，可令一名角色摸一张牌；③你回合外不因【玲珑】失去牌后，可弃置一名角色区域内一张牌。<br><br>奇巧：你首次执行【玲珑】其中一项效果时手牌上限加一，回合结束后重置【奇巧】。",
yunquhu:"驱虎",
"yunquhu_info":"出牌阶段限一次：你可选择与一名此回合未对其发动过【驱虎】角色拼点，若你赢：你恢复一点体力，然后令其对你指定的一名角色造成一点伤害；若你没赢：其对你造成一点伤害，然后重置【驱虎】本回合使用次数。",
yunwangzuo:"王佐",
"yunwangzuo_info":"锁定技：你受到伤害后摸X张牌，然后可令一名角色将手牌补充至体力上限（至多为五），若其手牌数不小于体力上限，则改为摸X张牌且此回合不能再对其发动【王佐】。（X为你已损失体力值）",
yunqiaobian:"巧变",
...
"yunlingrenxiaoxiong_info":"锁定技：你的手牌上限始终不小于体力上限，场上有角色死亡后你重置武将牌并恢复一点体力。",
yunlingren:"凌人",
"yunlingren_info":"一回合限一次：你使用带有〖伤害〗标签的牌指定目标后，可猜测其中的一个目标的手牌中是否有基本牌、锦囊牌、装备牌，若猜中的项目数不小于一：你摸猜中项目数的牌；不小于二：此牌对其造成的伤害加一；不小于三：你随机获得一个『魏』势力角色的技能直到你再次因【凌人】获得技能。（觉醒技、隐匿技、使命技除外）<br><br>枭雄：你的手牌上限始终不小于体力上限，场上有角色死亡后你重置武将牌并恢复一点体力。",
yunkunlong:"困龙",
"yunkunlong_info":"我们中出了一个奸细！",
yunjiwue:"极武",
"yunjiwue_info":"你使用【杀】或【决斗】指定目标时，可弃置一张手牌令此【杀】或【决斗】对目标造成的伤害加一；若目标因此进入濒死状态，你令其死亡。",
yunyugui:"驭鬼",
...
yunchuandao:"传道",
"yunchuandao_info":"锁定技：你始终处于横置状态；一名其他角色：①准备阶段时，你令其横置；②弃牌阶段时，其可交给你一张牌然后你可令其恢复一点体力。",
yuntaiping:"太平",
...
yunqianya:"谦雅",
"yunqianya_info":"①摸牌阶段前你可弃置一张牌并跳过此阶段，然后令一名其他角色立即执行一次额外的摸牌阶段；②其他角色使用锦囊牌指定你为唯一目标时，你可弃置一张牌令一名其他角色立即执行一次额外的出牌阶段；③弃牌阶段后你可弃置一张牌令一名其他角色立即执行一次额外的弃牌阶段，若你已受伤，则获得一点护甲（护甲至多不超过你已损失体力值）。",
yunguose:"国色",
...
yundengwei:"登位",
"yundengwei_info":"觉醒技：结束阶段，若你发动过至少五次【夺嫡】或因【行殇】继承了［主公］身份，则你增加一点体力上限并恢复一点体力，失去【夺嫡】【行殇】获得【承天】；你身份为［主公］时额外获得【颂威】。",
yunchengtian:"承天",
...
yunfanghun:"芳魂",
"yunfanghun_info":"准备阶段你可从随机X名『蜀』势力角色中选择获得至多X个主公技、觉醒技、限定技、隐匿技、使命技以外的技能直到你再次因【芳魂】获得技能。（X为你当前体力值，最多为五）",
yunyazi:"睚眦",
...
yunpoqian:"破堑",
"yunpoqian_info":"场上角色受到不为你造成的伤害后，你可令伤害来源获得一枚【堑】（最多能获得三个【堑】）。锁定技：①有【堑】的角色摸阶段移除一枚【堑】，摸牌数减半（向上取整），然后你摸减少数的牌；②一名角色回合结束后，若此回合内有携带【堑】的角色死亡，你获得一个额外回合。",
yunliegong:"烈弓",
...
yunxuezhao:"血诏",
"yunxuezhao_info":"每回合限一次：你使用非转化的基本牌或普通锦囊牌指定唯一角色为目标时，若你体力值大于一，则你可以失去一点体力选择令至多X名其他角色视为对此牌目标使用一张同名牌。（X为你已损失体力值，至多为三）",
yuntianming:"天命",
"yuntianming_info":"锁定技：结束阶段，若你已损失体力值不小于体力值，则恢复Y点体力、摸Y张牌。（Y为你已损失体力值的一半，向上取整）",
yunhanzuo:"汉祚",
...
yunzhenjun:"镇军",
"yunzhenjun_info":"你使用黑色牌指定一名体力值大于你的角色为目标时，可令其失去一点体力。",
yunyizhong:"毅重",
"yunyizhong_info":"出牌阶段限一次：你可选择一名其他角色令其将手牌数摸至或弃至与你相同。（最多摸五张）",
yunmansi:"蛮嗣",
"yunmansi_info":"①出牌阶段限一次：你可将任意张牌当做【南蛮入侵】对等量的角色使用；②你使用【南蛮入侵】造成伤害时，若目标体力值不小于你，则你可失去一点体力令此伤害加一；③锁定技：【南蛮入侵】对你无效。",
yunzhanyuan:"战缘",
"yunzhanyuan_info":"限定技：你进入濒死阶段时，可摸五张牌并将体力恢复至上限，然后你可与一名其他角色翻面。<br><br>龙凤巾：若未登场的武将中有〖阳·关索〗或〖天牢·鲍三娘〗：你可令【战缘】选择的男性／女性角色将武将牌替换为〖阳·关索〗／〖天牢·鲍三娘〗，并且你与其不会成为卡牌目标直到翻回正面。",
yunshiguan:"势冠",
"yunshiguan_info":"锁定技，每回合各限三次：①你的摸牌数为当前体力值（至多为五）；②你造成伤害后，若你的手牌数为场上最高，你须弃置一半（向下取整）手牌，否则你恢复一点体力。",
yunshiguana:"势冠",
"yunshiguana_info":"锁定技，每回合各限三次：①你的摸牌数为当前体力值（至多为五）；②你造成伤害后，若你的手牌数为场上最高，你须弃置一半（向下取整）手牌，否则你恢复一点体力。",
yunxiongjie:"雄桀",
"yunxiongjie_info":"一回合限一次，若你因【势冠】弃置了至少X张牌，则你可视为对至多X名角色使用一张【万箭齐发】。（X为你体力值的一半，向上取整）",
"yunlisi":"立嗣",
"yunlisi_info":"主公技：你的手牌上限翻倍；回合结束时，你可令一名其他角色获得【势冠】直到其回合结束，若其势力为『群』，则其因【势冠】首次弃置手牌或恢复体力时，你无视条件发动一次【势冠】②效果。",
yuntianyi:"天义",
"yuntianyi_info":"出牌阶段限一次，你可从牌堆／弃牌堆随机获得一张【杀】，然后用一张牌与至多三名角色拼点，若你赢：你本回合使用【杀】可额外指定其为目标；若你输：你本回合不能使用【杀】指定其为目标。",
yunaozhan:"鏖战",
"yunaozhan_info":"锁定技：①你使用【杀】造成伤害后摸一张牌，且使用【杀】次数额外加X（X为【天义】可额外指定的目标数，最多增加至三次）；②你拼点时，令对手随机选择一张手牌作为拼点牌。",
},
},
